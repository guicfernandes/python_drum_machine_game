...
active_instrument = [1 for _ in range(instruments)]
bpm = 240
playing = True
active_length = 0
active_beat = 0
...
def play_notes():
    for i in range(len(clicked)):
        if clicked[i][active_beat] == 1 and active_instrument[i] == 1:
            if i == 0:
                hi_hat.play()
            if i == 1:
                snare.play()
...
def draw_save_menu(beat_name, typing):
    ...
    saving_btn = pygame.draw.rect(
        screen, gray, [WIDTH // 2 - 200, HEIGTH * 0.75, 400, 100], 0, 5)
    saving_text = label_font.render('Save Beat', True, white)
    screen.blit(saving_text, (WIDTH // 2 - 70, HEIGTH * 0.75 + 30))

    exit_btn = pygame.draw.rect(
        screen, gray, [WIDTH-200, HEIGTH-100, 180, 90], 0, 5)
    exit_text = label_font.render('Close', True, white)
    screen.blit(exit_text, (WIDTH - 160, HEIGTH - 70))
...
def draw_load_menu(index):
    ...
    loading_btn = pygame.draw.rect(
        screen, gray, [WIDTH // 2 - 200, HEIGTH * 0.87, 400, 100], 0, 5)
    loading_text = label_font.render('Load Beat', True, white)
    screen.blit(loading_text, (WIDTH // 2 - 70, HEIGTH * 0.87 + 30))

    delete_btn = pygame.draw.rect(
        screen, gray, [(WIDTH//2) - 500, HEIGTH * 0.87, 200, 100], 0, 5)
    delete_text = label_font.render('Delete Beat', True, white)
    screen.blit(delete_text, ((WIDTH//2) - 485, HEIGTH * 0.87 + 30))
...
while(run):
    ...
    boxes = draw_grid(clicked, active_beat, active_instrument)
    ...
    beats_rect = pygame.draw.rect(
        screen, gray, [600, HEIGTH - 150, 200, 100], 5, 5)
    beats_text = medium_font.render('Beats In Loop', True, white)
    screen.blit(beats_text, (618, HEIGTH - 130))
    beats_text2 = label_font.render(f'{beats}', True, white)
    screen.blit(beats_text2, (680, HEIGTH - 100))
    beats_add_rect = pygame.draw.rect(
        screen, gray, [810, HEIGTH - 150, 48, 48], 0, 5)
    beats_sub_rect = pygame.draw.rect(
        screen, gray, [810, HEIGTH - 100, 48, 48], 0, 5)
    add_text2 = medium_font.render('+1', True, white)
    sub_text2 = medium_font.render('-1', True, white)
    screen.blit(add_text2, (820, HEIGTH - 140))
    screen.blit(sub_text2, (820, HEIGTH - 90))
